...
def get_devices():
    ...
    for l in lights:
        ...
        if 'hs' in attrs['supported_color_modes']:
            dev['rgb'] = attrs['rgb_color'] if attrs['rgb_color'] is not None else [
                0, 0, 0]
    ...
    if off == False:
        ...
        if color:
            if (temp == l['min_color_temp_kelvin'] or temp == l['color_temp_kelvin']):
                ...
                req.pop('kelvin')
        requests.post(
            f"https://danubio.ii.uam.es/api/services/light/turn_on", headers=headers, json=req)
    else:
        req = {'entity_id': data['entity_id']}
        requests.post(
            f"https://danubio.ii.uam.es/api/services/light/turn_off", headers=headers, json=req)
